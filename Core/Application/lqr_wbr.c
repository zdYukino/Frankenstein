...
void K_matrix_calc(lqr_data_t *data, float length)
{
    ...
    data->K11 = -3.5127f;//(                                 48.786f*length*length - 47.345f*length - 0.7115f); //R^2 = 0.9998 -6.6220f;-3.4474f;/
    data->K12 = -0.2617f;//(                                                       - 5.6196f*length + 0.1218f); //R^2 = 1      -0.5427f;-0.3050f;/
    data->K13 = -1.1791f;//( -55.064f*length*length*length + 42.892f*length*length - 11.463f*length - 1.1215f); //R^2 = 0.9991 -2.0980f;-1.9016f;/
    data->K14 = -1.3282f;//( -47.14f *length*length*length + 37.189f*length*length - 11.19f *length - 1.1837f); //R^2 = 0.9998 -2.4331f;-1.7822f;/
    data->K15 =  2.0445f;//( -180.55f*length*length*length + 159.05f*length*length - 53.184f*length + 8.1325f); //R^2 = 1       2.9590f;2.7861f;//
    data->K16 =  0.1950f;//( -8.0602f*length*length*length + 8.3599f*length*length - 3.4744f*length + 0.7793f); //R^2 = 1       0.4039f;0.2244f;//

    data->K21 = 3.0828f;//(  57.724f*length*length*length - 32.781f*length*length + 0.6436f*length + 3.5128f); //R^2 = 0.9998 1.6033f;1.5669f;//
    data->K22 = 0.3019f;//(  9.2032f*length*length*length - 7.2715f*length*length + 1.7563f*length + 0.236f ); //R^2 = 0.9969 0.1580f;0.2069f;//
    data->K23 = 2.1069f;//( -114.7f *length*length*length + 100.28f*length*length - 33.042f*length + 4.8043f); //R^2 = 1      1.0942f;1.6636f;//
    data->K24 = 2.2755f;//( -117.69f*length*length*length + 100.98f*length*length - 32.567f*length + 4.6866f); //R^2 = 0.9999 1.1753f;1.4948f;//
    data->K25 = 7.1210f;//(  379.24f*length*length*length - 293.52f*length*length + 78.959f*length + 6.6423f); //R^2 = 0.9992 9.5354f;8.5053f;//
    data->K26 = 0.3666f;//(  30.996f*length*length*length - 25.034f*length*length + 7.203f *length + 0.0868f); //R^2 = 0.9997 0.5415f;0.4300f;//
 }
/**
...
//                DDT_motor_toq_CTRL(&huart2, 0x02,  lqr_data_R.T_send);
                osDelay(4);
                DDT_motor_toq_CTRL(&huart2, 0x01,  VofaData[0]);
            }
            else {
